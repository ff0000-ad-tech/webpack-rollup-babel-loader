...
async function fixture(t, entry, options) {
  ...
  // fs.writeFileSync(
  //   path.join(__dirname, "src", "expected", entry),
  //   bundle.replace(/\r/g, "")
  // );
  // const sourcemap = mockFs.readFileSync("/bundle.js.map", "utf8");
  // fs.writeFileSync(
  //   path.join(__dirname, "src", "expected", `${entry}.map`),
  //   sourcemap.replace(/\\r/g, "")
  // );
...
test("plugins option", fixture, "fileLoader.js", {
  plugins: [commonjs({ extensions: [".js", ".jpg"] })]
});

test("external option", fixture, "external.js", {
  external: [path.join(__dirname, "src", "b.js")]
});
